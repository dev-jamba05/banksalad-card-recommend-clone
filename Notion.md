...
수백만 명의 트래픽을 감당할 수 있는 안정적인 카드 추천 서비스를 목표로 **Next.js (API Routes) + MySQL + Redis** 기반의 확장 가능한 백엔드 아키텍처를 구상했습니다.
...
### 🗄️ 데이터베이스 구조 (ERD)
사용자의 결제(소비) 데이터와 카드가 제공하는 혜택 데이터가 어떻게 매칭되는지 보여주는 핵심 구조입니다. 향후 관계형 데이터베이스(MySQL) 도입을 고려하여 정규화 및 외래키 설계를 진행했습니다.
...
> 💡 **MVP 구현 스펙:** 현재 배포된 MVP 버전은 무거운 물리적 DB 서버를 구축하는 대신, 위 **ERD 스키마를 TypeScript의 Interface와 Mock Data 객체(`src/data/mock.ts`)로 100% 치환하여 로컬 DB처럼 동작**하도록 구현했습니다. 추후 백엔드 데이터 소스가 연결되더라도 프론트엔드 비즈니스 로직 수정 없이 데이터를 교체할 수 있도록 확장성을 고려했습니다.

### ⚙️ 핵심 비즈니스 로직 (카드 추천 알고리즘)
단순한 화면 카피를 넘어, **'소비 금액별 최대 피킹률(혜택 효율)'**을 계산하는 추천 로직을 코드로 직접 구현했습니다.
1. **소비 데이터 그룹화 (Map-Reduce)**: 사용자의 결제 내역을 카테고리별로 합산.
2. **혜택 매칭 및 한도 적용**: 카드 혜택과 지출 매칭, 최대 할인 한도(`Math.min`) 보정.
3. **피킹률 산출**: `(예상 총 할인 금액 - 월환산 연회비) / 총 지출액 * 100` 계산.
4. **최적 정렬 (Sorting)**: 최종 절약 금액 기준 내림차순 정렬하여 상위 카드 렌더링.

### 🚀 트러블 슈팅 (성능 최적화 설계)
**이슈 1: 카드 및 혜택 조회 시 N+1 쿼리 문제 해결**
- **현상:** 유저 소비 내역과 500개 카드의 혜택을 매칭할 때, 각 카드별로 혜택을 조회하는 쿼리가 루프 내에서 발생하여 API 응답이 매우 지연됨.
- **해결 방안:** ORM의 `Fetch Join` (또는 Prisma `include`)을 활용하여 `CARD`와 `CARD_BENEFIT` 테이블을 1회의 단일 쿼리로 묶어서 가져오도록 최적화.

**이슈 2: 실시간 피킹률 연산 병목 현상 및 캐싱 최적화**
- **현상:** 사용자가 앱에 접속할 때마다 대량의 결제 내역을 실시간으로 연산하면 서버 CPU 리소스 고갈 및 DB 부하 심화.
- **해결 방안:** 잦은 변동이 없는 카드사 혜택 데이터는 **Redis**에 캐싱하여 DB I/O를 최소화하고, 실시간 연산 대신 트래픽이 적은 새벽에 **배치(Batch)** 처리하여 맞춤 피킹률을 사전 연산하도록 설계.
